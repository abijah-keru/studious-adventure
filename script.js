const activities = [
    ...
    {
        id: 56,
        name: "We Belong to Time",
        description: "An art exhibition exploring themes of time, belonging, and contemporary expression.",
    ...
    },
    {
        ...
        calendarStart: "2025-11-23T11:00:00+03:00",
        calendarEnd: "2026-01-05T17:00:00+03:00"
    },
    {
        id: 50,
    ...
    },
    {
        id: 58,
        name: "Harmony",
        description: "An art exhibition celebrating balance, unity, and artistic harmony through diverse contemporary works.",
        location: "One Off Contemporary Gallery",
        ...
        price: "Free Entry\nOpening Hours: Tue to Sat 10am – 5pm, Sun 12pm – 5pm",
        calendarStart: "2025-11-29T10:00:00+03:00",
        calendarEnd: "2026-01-18T17:00:00+03:00"
    },
    {
        id: 59,
        name: "Pirikania; Shades of Pain and Progress",
        description: "A powerful art exhibition exploring themes of struggle, resilience, and transformation through contemporary visual narratives.",
        location: "One Off Contemporary Gallery",
        latitude: -1.217239382418902,
        longitude: 36.791280998896106,
        schedule: "Nov 29 - Jan 18, 2026",
...
const SHARE_BASE_URL = 'https://nairobidates.co.ke';
...
function buildActivityLink(slug, activityId, name) {
    const cleanBase = SHARE_BASE_URL.replace(/\/$/, '');
    const targetSlug = slug || (activityId ? `activity-${activityId}` : (slugify(name || '') || 'event'));
    return `${cleanBase}/?event=${encodeURIComponent(targetSlug)}`;
}
...
if (initialDeepLinkSlug) {
    ...
    if (initialActivity) {
        const initialShareUrl = buildActivityLink(initialDeepLinkSlug, initialActivity.id != null ? String(initialActivity.id) : '', initialActivity.name || '');
...
function getDeepLinkSlugFromUrl(url) {
    ...
    const querySlug = currentUrl.searchParams.get('event') || currentUrl.searchParams.get('slug');
    if (querySlug) {
        return decodeURIComponent(querySlug);
    }
    if (currentUrl.hash) {
        const hashMatch = currentUrl.hash.match(/events\/([^/]+)/i);
...
function focusActivityBySlug(slug, attempt = 0) {
    ...
    if (activity) {
        const shareUrl = buildActivityLink(slug, activity.id != null ? String(activity.id) : '', activity.name || '');
...
function getBulletPoints(activity) {
    ...
    if (name === 'thursday night live') {
      ...
      // Sax & Violins (Nairobi Street Kitchen)
